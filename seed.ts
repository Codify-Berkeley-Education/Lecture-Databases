...
const seed = async () => {
	const result = await db.transaction(async (db) => {
		...
		const userResults = await db.insert(usersTable).values(users).returning();

		// Create 10 posts
		const posts: InferInsertModel<typeof postsTable>[] = [];
		for (let i = 0; i < 10; i++) {
			posts.push({
		...
		const postResults = await db.insert(postsTable).values(posts).returning();
		...
		// Create 20 comments
		const comments: InferInsertModel<typeof commentsTable>[] = [];
		for (let i = 0; i < 20; i++) {
			comments.push({
				content: faker.lorem.paragraph(),
				authorId:
